...
def stochastic_sim_v4(params):
    ...
    while m_accum < TARGET_MASS and t < 1000:
        ...
        # 3. 可用性计算：引入故障停机时间损失
        # 假设小故障平均损失 DOWNTIME_SMALL，大故障损失 DOWNTIME_BIG
        total_downtime = (num_small * DOWNTIME_SMALL) + (num_big * DOWNTIME_BIG)
        
        # 可用性系数：1.0 表示全年满负荷，0.1 为保底运行率
        availability = max(0.1, 1.0 - total_downtime)
