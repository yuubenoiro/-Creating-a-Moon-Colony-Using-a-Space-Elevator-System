...
class DecisionAnalyzerV4:
    ...
    def evaluate_strategy(self, total_mass, r_usage, alpha=0.0, iterations=50):
        ...
        #print("volatility_risk : ",volatility_risk)
        
        # 2. 结构性冗余风险 (过度依赖单一方式)
        redundancy_risk = 40 * (1 - r_usage)
        #print("redundancy_risk", redundancy_risk)

        # 3. 黑碳诱发环境风险 (核心新增)
        # 映射 r_usage 到黑碳强度并计算平流层扰动
        bc_x = r_usage * 5.0
        strat_disturb = self.model.calculate_stratospheric_disturbance(bc_x)
        #print("strat_disturb:", strat_disturb)
        # 扰动指标 y 越大，环境安全性分值越高（惩罚越大）
        # y0=0.1 时基本无惩罚，y 接近 1.3 时产生约 30 分的额外风险
        env_safety_risk = exp((strat_disturb-0.5)*5)
        #print("env_safety_risk", env_safety_risk)
        
        # 综合风险评分：加权组合
        total_risk = np.clip(((volatility_risk /3  * 0.1 + redundancy_risk/20 * 0.45 + env_safety_risk/15 * 0.45)-0.5)*100, 0, 100)
