...
class EnvironmentalImpactModel:
    ...
    @staticmethod
    def _bc_climate_ode(y, t, E_val):
        """黑碳-温度-反照率耦合微分方程"""
        B, T, Bs = y
        dBdt = E_val - (0.2 + 0.02 * T) * B
        albedo = A0_ALBEDO * np.exp(-0.5 * Bs)
        dTdt = (1/C_HEAT) * (ALPHA_BC * np.log(1 + BETA_BC * B) + 
                             4.0 * (A0_ALBEDO - albedo) - LAMBDA_F * T)
        dBsdt = PHI_DEP * B - (PSI0 * np.exp(KAPPA_T * T)) * Bs
        return [dBdt, dTdt, dBsdt]
    ...
    def calculate_dynamic_eta(self, bc_intensity):
        """
        根据黑碳强度计算动态效率 η
        bc_intensity: 对应模型中的 x (归一化强度)
        """
        # 1. 能见度影响 (Sensor/Control Loss)
        # 映射 x 到 bc_emission (ug/m3) 假设系数为 20
        bc_ug_m3 = bc_intensity * 20 
        
        b_ext = self.params.VIS_ALPHA * (bc_ug_m3 ** self.params.VIS_DELTA)
        visibility = 3.912 / (b_ext + 0.01)
        eta_vis_factor = (1 - np.exp(-visibility / self.params.V_THRESHOLD))
        
        # 2. 平流层扰动影响 (Mechanical Vibration Loss)
        y_strat = 0.1 + self.params.STRAT_A * (1 - np.exp(-self.params.STRAT_ALPHA * bc_intensity))**self.params.STRAT_GAMMA
